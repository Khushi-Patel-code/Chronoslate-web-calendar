...
let bookings = [];   // bookings array to store bookings data 
try {
    bookings = JSON.parse(localStorage.getItem("bookings")) || [];
} catch (error) {
    console.error("Error loading bookings from the localStorage:", error);
...
function generateCalendar(month, year) {
    // Get the first day of the month and the num of days in month
...
function loadCalendarBookings(cell , day, month, year) { 
    bookings.forEach(booking => {
        ...
        if (eventDay === day && eventMonth - 1 === month && eventYear === year) {
            ...
            cell.title = `${booking.name}`; // Display event name when hovered

            cell.addEventListener("click", function () {
                if (confirm(`Are you sure you want to delete this booking on: ${booking.date}?\nbooking name: ${booking.name}\nbooking Reason: ${booking.reason}`)) {
                    try {
                        bookings = bookings.filter(b => b !== booking);   // Remove booking from bookings array in localstorage
...
function dateClick(cell, day , month , year) {
    clearForm();   // Clear the previsou form data
...
function saveEvent(event){
    ...
    if (eventName === "") {
        ...
        return;  //event name required for booking
    }
...
function clearForm(){
    document.getElementById("event-form").reset();  //sets all the forms values to empty
    selectedDate = null;
